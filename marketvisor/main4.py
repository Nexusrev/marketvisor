...
from InquirerPy.validator import FilePathValidator
import pandas as pd
import numpy as np
...
def indicators_menu():
    ...
    if Confirm.ask("[bold cyan]Configure Simple Moving Average (SMA)?[/bold cyan]"):
        window = Prompt.ask("[bold cyan]Enter window size for SMA[/bold cyan]", default="20")
        indicators['SMA'] = window
    if Confirm.ask("[bold cyan]Configure Exponential Moving Average (EMA)?[/bold cyan]"):
        span = Prompt.ask("[bold cyan]Enter span for EMA[/bold cyan]", default="20")
        indicators['EMA'] = span
    if Confirm.ask("[bold cyan]Configure Relative Strength Index (RSI)?[/bold cyan]"):
        window = Prompt.ask("[bold cyan]Enter window size for RSI[/bold cyan]", default="14")
        indicators['RSI'] = window

    if Confirm.ask("[bold cyan]Configure Moving Average Convergence Divergence (MACD)?[/bold cyan]"):
...
def main_menu():
    return inquirer.select(
        message="[bold magenta]Select a Module:[/bold magenta]",
